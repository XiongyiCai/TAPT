...
class KubricDataset(torch.utils.data.Dataset):
    def __init__(self, seqlen = 8,mode = 'clip', dataset = 'train'):
        
        files = os.listdir('../handle_movie/'+ dataset) 
        num_file = len(files)
        

        data = []
        for i in range(num_file):
            path = '../handle_movie/'+ dataset+ '/movie_'+str(i).rjust(5,"0")+'.pkl'
    ...
    def __getitem__(self, index):
        ...
        if self.dataset == 'validation_with_query' or self.dataset == 'train_with_query':
            ...
            if self.mode == 'seqlen':
                ...
                query_points = self.data_to_gt[subfolder_id]['query_points'] # N, 3
            else:
                print('unknown mode')
